...
class Avl
{
     ...
     * @return bool
     */
    public function isAvlTree(Node $root)
    {
        if ($root == null) {
            return true;
        }
        ...
        // 平衡因子
        $bf = $this->getdepth($root->left) - $this->getdepth($root->right);

        if ($bf > 1 || $bf < -1) {
            return fasle;
        }

        return $this->isAvlTree($root->left) && $this->isAvlTree($root->right);
    }
     ...
     * @return bool
     */
    public function isAvlTreeByStack(Node $root)
    {
        $flag = true;
